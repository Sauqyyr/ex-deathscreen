...
RegisterNetEvent('nc-deathscreen:openUI', function(time)
    isDead = true
...
RegisterNetEvent('nc-deathscreen:revive', function()
    isDead = false
...
RegisterNetEvent('nc-deathscreen:die', function(time)
    isDead = true
    SendNUIMessage({action = "DSMenu", open = true, time = time})
    if Config.Blur then
        TriggerScreenblurFadeIn()
...
RegisterNetEvent('nc-deathscreen:res', function()
    SendNUIMessage({action = "res"})
end)

RegisterNetEvent('nc-deathscreen:updateRes', function(time)
    SendNUIMessage({action = "updateRes", time = time})
end)

RegisterCommand('test',function()
    TriggerEvent('nc-deathscreen:updateRes', 0)
end)
